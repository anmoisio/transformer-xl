...
def rescore(corpus, nbest_file, model, ext_len, mem_len, re):
...
parser.add_argument('--nbest-file', type=str, default='../data/nbests/yle_test_nbest_50_after_LSTM_lattice',
...
with open(args.nbest_file, "r", encoding="utf-8") as reader:
    while True:
        ...
        if len(Splitted) == 1:
            Splitted.append(args.sent_sep)

        all_ids.append(Splitted[0])
        tmpfile.write(Splitted[1] + '\n')
...
for model_dir in model_dirs:
    ...
    re = open(os.path.join(args.out_dir, os.path.basename(args.nbest_file) + '-' + model_dir), 'w', encoding='utf-8')
